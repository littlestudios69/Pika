...
module.exports.execute = async(bot, msg, args, data) => {
    ...
    if(member.id === msg.author.id) return msg.send("Why would you kick yourself?");
    if(member.id === bot.user.id) return msg.send("Why would you kick me?");
    if(member.id === msg.guild.ownerID) return msg.send("You can't kick the owner.");
